...
void mouse(int button, int state, int x, int y) { // x and y (Origin (0,0) at the top-left corner of the window.x increases to the right.y increases downwards.) are the mouse coordinates in pixels (from 0 to 699 px)
...
void updateAvailableMoves() {
    ...
    for (int r = 0; r < BOARD_SIZE; r++)
        for (int c = 0; c < BOARD_SIZE; c++)
            if (isLegalMove(board, selectedRow, selectedCol, r, c, currentPlayer)) // Getting the available moves for the selected piece and flip the corresponding values of the squares in the availableMoves array to true
